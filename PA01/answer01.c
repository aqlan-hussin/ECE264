...
double simpson_numerical_integration(integrand intg_arg)
{
   ...
   for (count = 0; count < intg_arg.n_intervals; count++) {
     integral = integral + ((((intg_arg.lower_limit + (count + 1)*width) - (intg_arg.lower_limit + count*width)) / 6) * (intg_arg.func_to_be_integrated((intg_arg.lower_limit + count*width)) + 4*intg_arg.func_to_be_integrated(((intg_arg.lower_limit + count*width)+(intg_arg.lower_limit + (count + 1)*width))/2) + intg_arg.func_to_be_integrated((intg_arg.lower_limit + (count + 1)*width))));
   }
   return integral;
}
// do not change this
...
double simpson_numerical_integration_for_fourier(integrand intg_arg, int n, 
   ...
   {
   double integral = 0.0;
   double period =  intg_arg.upper_limit - intg_arg.lower_limit;
   double width = (intg_arg.upper_limit - intg_arg.lower_limit) /  intg_arg.n_intervals;
   int count;

   for (count = 0; count < intg_arg.n_intervals; count++) {
     integral = integral + (((intg_arg.lower_limit + (count + 1)*width) - (intg_arg.lower_limit + count*width)) / 6) * ((intg_arg.func_to_be_integrated(intg_arg.lower_limit + count*width)*cos_sin(n * M_PI * (intg_arg.lower_limit + count*width)/(period/2)))) + 4*(intg_arg.func_to_be_integrated(((intg_arg.lower_limit + count*width)+(intg_arg.lower_limit + (count + 1)*width))/2)*cos_sin(n* M_PI * (((intg_arg.lower_limit + count*width)+(intg_arg.lower_limit + (count + 1)*width))/2)/(period/2)) + (intg_arg.func_to_be_integrated(intg_arg.lower_limit + (count + 1)*width)*cos_sin(n * M_PI * (intg_arg.lower_limit + (count+1)*width)/(period/2))));
   }
...
void fourier_coefficients(fourier fourier_arg) 
{
   ...
   for (i = 0; i < fourier_arg.n_terms; i++) {
      ...
      if (i == 0) {
	fourier_arg.a_i[i] = (2/period) *  simpson_numerical_integration(fourier_arg.intg);
      }

      else {
	fourier_arg.a_i[i] = (2/period) *  simpson_numerical_integration_for_fourier(fourier_arg.intg, i, cos);
      }

      fourier_arg.b_i[i] = (2/period) *  simpson_numerical_integration_for_fourier(fourier_arg.intg, i, sin);
   }
}
#endif /* NTEST_FOURIER */
